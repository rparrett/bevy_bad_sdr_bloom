...
use bevy::math::UVec2;
use bevy::prelude::{IntoSystemAppConfig, IntoSystemConfig};
...
use bevy::render::{ExtractSchedule, RenderSet};
#[cfg(feature = "trace")]
use bevy::utils::tracing::info_span;
use bevy::utils::HashMap;

const BLOOM_SHADER_HANDLE: HandleUntyped =
    HandleUntyped::weak_from_u64(Shader::TYPE_UUID, 8661849338007983600);
...
impl Plugin for BloomPlugin {
    fn build(&self, app: &mut App) {
        ...
        render_app
            ...
            .add_system(extract_bloom_settings.in_schedule(ExtractSchedule))
            .add_system(prepare_bloom_textures.in_set(RenderSet::Prepare))
            .add_system(prepare_bloom_uniforms.in_set(RenderSet::Prepare))
...
impl Node for BloomNode {
    ...
    fn run(
        ...
        {
            let view = &BloomTextures::texture_view(&textures.textures_a[0]);
        ...
        for mip in 1..textures.mip_count {
            let view = &BloomTextures::texture_view(&textures.textures_a[mip as usize]);
        ...
        for mip in (1..textures.mip_count).rev() {
            let view = &BloomTextures::texture_view(&textures.textures_b[mip as usize - 1]);
...
struct BloomTextures {
    textures_a: Vec<CachedTexture>,
    textures_b: Vec<CachedTexture>,
    mip_count: u32,
}

impl BloomTextures {
    fn texture_view(texture: &CachedTexture) -> TextureView {
    ...
    for (entity, camera) in &views {
        ...
        }) = camera.physical_viewport_size
        {
            ...
            commands.entity(entity).insert(BloomTextures {
                textures_a: texture_a_vec,
                textures_b: texture_b_vec,
                mip_count,
            });
        }
    }
...
fn queue_bloom_bind_groups(
    ...
    if let Some(uniforms) = uniforms.uniforms.binding() {
        for (entity, view_target, textures) in &views {
                    ...
                    entries: &[
                        BindGroupEntry {
                            ...
                            resource: BindingResource::TextureView(&BloomTextures::texture_view(
                                &textures.textures_a[mip as usize - 1],
                            )),
                        },
                        BindGroupEntry {
                            binding: 1,
            ...
            for mip in 1..textures.mip_count {
                let up = BloomTextures::texture_view(&textures.textures_a[mip as usize - 1]);
                let org = BloomTextures::texture_view(if mip == textures.mip_count - 1 {
                    &textures.textures_a[mip as usize]
                } else {
                    &textures.textures_b[mip as usize]
                });
